...
//createLoggingPane();
  var lc = getStyle('palette','color');
  if (lc) {
    lc = Color.fromString(lc).toHexString();
    linecolor = '&line=' + lc.replace('#','');
  }
  var fc = getStyle('palette','background-color');
  if (fc) {
    fc = Color.fromString(fc).toHexString();
    fillcolor = '&fill=' + fc.replace('#','');
  }
  Palette = new Array();
  setupPalette();
  DoResize();
...
//  if (code<256) {return 0;}
  ...
  if (code.length==3){
    ...
    RotKey = keys[tGrp][tSym][5];
    FilKey = keys[tGrp][tSym][4];
...
function PaletteSymbol(row,col){
  ...
  function fnRefresh(){
    htmlPS = "";
    if (this.code) {
      htmlPS = '<img src="glyph.php?key=' + this.code + '&size=' + this.size + linecolor + fillcolor + '">';
    }    
  ...
  function fnDragging(){
    htmlPS = "";
    if (this.code) {
      htmlPS = '<img src="glyph.php?key=' + this.code + linecolor + fillcolor +  '">';
    }    
    ...
    if (display==1) {
      ...
      if ((this.row<inCol) && (idPS<=display)) {
        ...
        if ((selected<11) && (idPS>1)){
          this.code=keys[selected-1][idPS-1][1]
        } else {
          this.code=keys[selected-1][idPS-1][1]
        }
      }
...
//    nCode = keys[grp-1][selected-1][0] + id;
    nCode = keys[grp-1][selected-1][0] + dechex(this.col) + dechex(this.row);
    if (validkey(nCode)){
      this.grp=grp;
      this.sym=selected;
      this.dtl=1;
