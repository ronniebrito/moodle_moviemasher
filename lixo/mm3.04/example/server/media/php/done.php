...
if (! $err)
{
	...
	$options['CoderErrorURL'] = 'http://' . $user_pass . $host . '/' . $path_cgi . 'error.php?id=' . $id;
	
	// specify URL to hit in case of encoding error
	$options['CoderDoneURL'] = 'http://' . $user_pass . $host . '/' . $path_cgi . 'addmedia.php?id=' . $id;
	
	if ($moviemasher_client->progressesLocally())
	{
		$options['CoderProgressURL'] = 'http://' . $user_pass . $host . '/' . $path_cgi . 'progress.php?id=' . $id;
	}
	...
	{
		if (! $renders_locally) $options['CoderFileURL'] .= $user_pass . $host . '/' . $path_cgi . 'encoded.php?id=' . $id;
		else $options['CoderFileURL'] .= $path_media . $id;		
	}
	else 
	{
