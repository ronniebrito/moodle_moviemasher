...
*/
	public class Picturebox extends Composite
	{
		...
		override protected function _setCompositedSize():Boolean
		{
			...
			try
			{
				if ((__composited != null) && (__module != null))
				{
					...
					__module.metrics = new Size(scale[0], scale[1]);
					...
					__module.displayObject.x = Math.round((w / 2) + position[0]) + (has_back ? Math.round((_backWidth - w) / 2) : 0);
					__module.displayObject.y = Math.round((h / 2) + position[1]) + (has_back ? Math.round((_backHeight - h) / 2) : 0);
		...
		protected function _backSize():void
		{
			try
			{
				var per:Number = _getFade(_frame) / 100;
