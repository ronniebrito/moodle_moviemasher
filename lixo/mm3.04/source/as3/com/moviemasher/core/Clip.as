			...
			try
			{
				switch(property)
				{
					...
					case ClipProperty.LENGTHSECONDS:
						value = new Value(RunClass.TimeUtility['timeFromFrame'](lengthFrame, __mash.getValue(MashProperty.QUANTIZE).number));
						break;
					...
					case MediaProperty.DURATION: // number of frames in mash quantization of media as a float
						value = new Value(RunClass.TimeUtility['frameFromTime'](__mediaLength, mash.getValue(MashProperty.QUANTIZE).number, ''));
						break;
				...
				switch(property)
				{
					...
					case ClipProperty.LENGTHSECONDS:
						if ((__type == ClipType.IMAGE) || __media.isModular)
						{
							if (value.boolean)
							{
								lengthFrame = Math.round(RunClass.TimeUtility['frameFromTime'](value.number, __mash.getValue(MashProperty.QUANTIZE).number)); // collision detection for effects
...
*/
		...
		{
			var media_frames:Number = RunClass.TimeUtility['frameFromTime'](__mediaLength, mash.getValue(MashProperty.QUANTIZE).number) * __speed;
			return new Value(((__startTrimFrame * 100) / media_frames) + ',' + ((__endTrimFrame * 100) / media_frames));
		}
