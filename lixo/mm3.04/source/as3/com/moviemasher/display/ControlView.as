	...
	public class ControlView extends View
	{
		...
		public function initDispatchers():void
		{
			try
			{
				...
				for (var attribute_index:int = 0; attribute_index < b; attribute_index++)
				{
		...
		private function __visibleEnable():Boolean
		{
			...
			{
				RunClass.MovieMasher['msg'](this + '.__visibleEnable', e);
			}
		...
		private function __evaluateExpression(expression:String):Boolean
		{
			...
			if (expression.length)
			{
				...
				var elements:Array = expression.split(/([\w\.]+)([><!]?[=]?)([\w\.]+)/g);
				...
				if (z == 5)
				{
					...
					if (bits.length > 1)
					{
						var property:String = bits.pop();
						var ob:IValued = RunClass.MovieMasher['getByID'](bits.join('.')) as IValued;
						if (ob != null)
						{
							...
							if ( (test_value == 'undefined') || (test_value == 'null')  || (test_value == 'empty') )
							{
								...
								var is_undefined:Boolean = ob.getValue(property)[((test_value == 'empty') ? 'empty' : 'undefined')];
							...
							{
								//if (expression == 'player.dirty=0') RunClass.MovieMasher['msg(expression + ' ' + ob + ' ' + property + ' ' + ob.getValue'](property).string);
								property = ob.getValue(property).string;
								switch (elements[2])
								{
									case '>' :
		...
		private function __evaluateProperty(property:String):Boolean
		{
			...
			if (! value.empty)
			{
				...
				value.delimiter = (and_search ? '&' : '|');
				var expressions:Array = value.array;
