	...
	public class Timeline extends ControlPanel
	{
		...
		override protected function _createChildren():void
		{
			try
			{
				...
				__tracks = new Object();
				__tracks.clip = getValue('cliptracks').number;
				__tracks.audio = getValue('audiotracks').number;
				__tracks.video = getValue('videotracks').number;
				__tracks.effect = getValue('effecttracks').number;
		...
		override public function dragOver(drag:DragData):Boolean
		{
			...
			try
			{
				if ((_mash != null) && (items[0] is IClip))
				{
					...
					if (clip_type == ClipType.MASH)// && (_mash.getValue(CommonWords.ID).equals(clip.getValue(CommonWords.ID).string)))
					{
						// at the moment mashes can't be embedded in other mashes
						return false;
					}
					var not_visual:Boolean = (! clip.isVisual());
		...
		public function pixels2Track(y_pixels:Number, type:String, lowest_track:int = 0):int
		{
			...
			if (__tracks[type])
			{
				switch (type)
				{
					...
					case ClipType.AUDIO :
						...
						if (__tracks.video && RunClass.Media['multitrackVideo'])
						{
			...
			track = Math.min(__tracks[type] + (getValue(type + 'tracks').number ? 0 : 1), track);
		...
		override public function downPreview(preview:IPreview, event:MouseEvent):void
		{
			if ( ! (getValue('notrim').boolean && getValue('nodrag').boolean))
			{
				...
				try
				{
					...
					{
						__trimStart(event, on_handle, _selection.firstItem() as IClip);
						do_press = false;
					}
					if (do_press)
					{
		...
		private function __track2Pixels(track:Number, type:String):Number
		{
			...
			switch (type)
			{
				...
				case ClipType.AUDIO :
					...
					pixels += (track + ((__tracks.video && RunClass.Media['multitrackVideo']) ? 0 : -1) ) * typeHeight(ClipType.AUDIO);
		...
		private function __createClip(clip:IClip, container:DisplayObjectContainer = null):IPreview
		{
			...
			try
			{
				...
				if (container != null)
				{
					preview = _instanceFromOptions(_previewOptions(clip), clip.media.tag, clip);
					if (preview != null)
					{
		...
		private function __calculateContentHeight():void
		{
			...
			if (__tracks.video)
			{
				...
				if (RunClass.Media['multitrackVideo'])
				{
		...
		private function _previewOptions(clip:IClip):IOptions
		{
			...
			{
				RunClass.MovieMasher['msg'](this + '._previewOptions', e);
			}
			return options;
		}
		override protected function _previewData(clip:IClip):Object
		{
			...
			try
			{
				...
				//RunClass.MovieMasher['msg'](this + '._previewData leftcrop = ' + object['leftcrop'] + ' options_x = ' + options_x);
		...
		private function __resetTracks():void
		{
			...
			{
				var cliptracks:Number = getValue('cliptracks').number;
				...
				__tracks.video = (videotracks ? 1 : _mash.getValue(ClipType.VIDEO).number);
				__tracks.audio = (audiotracks ? audiotracks : _mash.getValue(ClipType.AUDIO).number);
				__tracks.effect = (effecttracks ? effecttracks : _mash.getValue(ClipType.EFFECT).number);
				...
				for (var kk = 0; kk < 4; kk++)
				{
					...
					i = 0;
					z =  __tracks[k];
					if ((k == ClipType.AUDIO) && RunClass.Media['multitrackVideo'] && __tracks.video)
					{
						z++;
					}
		...
		private function __trimStart(event:MouseEvent, direction : Number, clip : IClip):void
		{
			var i : Number;
			var items;
		...
		private function __viewableClips():Array
		{
			...
			if (_mash != null)
			{
				...
				for (var j:int = 0; j < 3; j++)
				{
					...
					if ((type == types[1]) && RunClass.Media['multitrackVideo'])
					{
