...
* All Rights Reserved.
*/
include_once('MovieMasher/Coder.php');
...
class MovieMasher_Coder_Decoder extends MovieMasher_Coder
{
	...
	function _codeFile()
	{
		...
		$xml_string = @file_get_contents($this->_options['DecoderConfigURL']);
	...
	function __buildFlash()
	{
		...
		if (! empty($this->__mashData['timespans']))
		{
			...
			while ($this->__flashIsProcessing()) 
			{
				...
				else
				{
					if ((time() - $started) > $this->_options['FlashSeconds'])
					{
						...
						$err = @file_get_contents($this->_buildDir . 'error.txt');
			...
			if (file_exists($this->_buildDir . 'error.txt')) throw new RuntimeException(@file_get_contents($this->_buildDir . 'error.txt'));
