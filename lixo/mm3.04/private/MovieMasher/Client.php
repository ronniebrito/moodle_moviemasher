...
include_once('MovieMasher/Coder/Decoder.php');
include_once('MovieMasher/Coder/Encoder.php');
...
class MovieMasher_Client extends MovieMasher
{
	...
	{
		$val = parent::getOption($name);
		
		if (empty($val))
		{
			$val = $this->getCoderOption($name);
		}
		return $val;
	}
	function post($type) 
	...
	function _populateDefaults()
	{
		...
		$this->_configDefaults['LogRequests'] = array(
			'value' => 'BOOLEAN',
			'description' => "Whether or not to add requests to the log.",
			'default' => 1,
			'empty_ok' => 1,
		);
		$this->_configDefaults['LogResponses'] = array(
			'value' => 'BOOLEAN',
			'description' => "Whether or not to add responses to the log.",
			'default' => 1,
			'empty_ok' => 1,
		);
	}
