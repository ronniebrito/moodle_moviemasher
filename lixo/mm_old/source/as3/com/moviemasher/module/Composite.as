...
* @see Mash
*/
	public class Composite extends Module
	{
		...
		override public function setFrame(clip_frame:Number):Boolean
		{
			...
			try
			{
				if (__module == null)
				{
					...
					{
						__module = __composited.module;
					}
				}
				if ((__module != null) && (! _displayObjectContainer.contains(__module.displayObject)))
				{
					_displayObjectContainer.addChild(__module.displayObject);
				}

				if ((__composited != null) && (__module != null))
				{
					changed = _setCompositedSize();
					if (! _mute) __module.volume = __composited.volumeFromTime(_compositedFrame(), _volume * 100);
					...
					__module.displayObject.blendMode = blend;
				}
				else
				{
					if (__composited == null)
		...
		protected function _setCompositedFrame(invert:Boolean = false):Boolean
		{
			...
			if ((__composited != null) && (__module != null))
			{
				try
				{
					...
					changed = __composited.setFrame(cf);
				}
				catch(e:*)
				{
		...
		protected function _setCompositedSize():Boolean
		{
			...
			if ((__composited != null) && (__module != null))
			{
				...
				{
					__module.metrics = _size;
					changed = (_setCompositedFrame() || changed);
					changed = (_setDisplayObjectMatrix(__module.displayObject) || changed);
				}
				catch(e:*)
				{
		...
		protected function _setDisplayObjectMatrix(display_object:DisplayObject):Boolean
		{
			var changed:Boolean = false;
			try
			{
				...
				for (var i:uint = 0; i < z; i++)
				{
					...
					if ((s != null) && s.length)
					{
						...
						if (per != 100)
						{
							default_values[key] = _getClipProperty(key + 'fade');
						}
					}
					else RunClass.MovieMasher['msg'](this + '._setDisplayObjectMatrix no value for ' + key);
				}
				changed = __applyTransform(per, display_object, apply_values, default_values);
			}
			catch(e:*)
			{
		...
		private function __applyTransform(per:Number, display_object:DisplayObject, apply_values:Object, default_values:Object):Boolean
		{
			var changed:Boolean = true; // TODO: actually check that matrix has changed
			try
			{
		...
		private function __unloadCompositedModule():void
		{
			try
			{
				if (__module != null)
				{
					if ((__module.displayObject != null) && _displayObjectContainer.contains(__module.displayObject))
					{
						_displayObjectContainer.removeChild(__module.displayObject);
					}
					__module = null;
				}
				if (__composited != null)
		...
		protected var __module:IModule;
