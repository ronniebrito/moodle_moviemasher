...
*/
	public class AVSequence extends AVAudio
	{
		...
		override public function buffer(first:Number, last:Number, mute:Boolean, rebuffer:Boolean):void
		{
			...
			var play_frames:Array = null;
			try
			{
				var url:String;
				var safe_key:String;
				var loader:IAssetFetcher;
				var frame_number:Number;
				var attempted:Dictionary = new Dictionary();
				
				play_frames = __playFrames(first, last);
				for each (frame_number in play_frames)
				{
					url = __urlFromTime(frame_number);
			...
			{
				RunClass.MovieMasher['msg'](this + '.buffer ' + first + '->' + last + ' ' + rebuffer + ' ' + play_frames, e);
			}
		}
			...
			try
			{
				if (is_buffered)
				{
					...
					for each (frame_number in play_frames)
					{
						...
						{
							is_buffered = false;
							//RunClass.MovieMasher['msg'](this + '.buffered ' + first + '->' + last + ' waiting for ' + url + ' ' + play_frames);
							break;
						}
					}
				}
		...
		override public function setFrame(clip_frame:Number):Boolean
		{
			...
			if (clip_frame >= 0)
			{
				...
				var play_frames:Array = __playFrames(clip_frame, clip_frame);
				clip_frame = play_frames.shift();
		...
		private function __graphicTimed(event:Event):void
		{
			try
			{
				...
				{
					var loader:IAssetFetcher = __loadingURLs.pop();
					var url:String = __loaders[loader];
					if (url != null)
					{
						...
						{
							//RunClass.MovieMasher['msg'](this + '.__graphicTimed could not get displayObject ' + url);
							__loadingURLs.unshift(loader);
						}
					}
				}
				if (! __loadingURLs.length)
