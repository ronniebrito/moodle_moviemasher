	...
	public class Chromakey extends Composite
	{
		...
		override protected function _changedSize():void
		{
			try
			{
				...
				addChild(__defringer);
				__defringer.x = __bitmap.x = - _size.width / 2;
				__defringer.y = __bitmap.y = - _size.height / 2;				
		...
		override protected function _clipPropertyDidChange(event:ChangeEvent):void
		{
			...
			try
			{
				switch (event.property)
				{
					case 'composites':
						_changedSize();
						// TODO was this okay to remove?? _initialize();
		...
		{ 
			try
			{
				...
				if ((__composited != null) && (__module != null))
				{
					...
					__module.displayObject.x = _size.width / 2;
					__module.displayObject.y = _size.height / 2;
					__render();					
					_displayObjectContainer.visible = false;
					__module.displayObject.x = 0;
					__module.displayObject.y = 0;
				}
			}
			catch(e:*)
			{
