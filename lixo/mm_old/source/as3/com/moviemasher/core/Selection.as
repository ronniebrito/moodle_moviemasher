	...
	public class Selection extends Propertied
	{
		...
		override public function setValue(value:Value, property:String):Boolean
		{
			...
			try
			{
				...
				if (items_with_property.length)
				{
					if (__action == null)
					{
						__action = new ClipsValueAction(items_with_property, property, value);
					}
					else
					{
		...
		private function __sharedValue(property:String):Value
		{
			...
			try
			{
				...
				for (i = 0; i < z; i++)
				{
					...
					{
						/*
						We shouldn't insist property be defined at this point, to support referencing it in hide, select, disable
						if (item.propertyDefined(property))
						{
						}
						*/
		...
		private function __dispatchChange():void
		{
			try
			{
				...
				dispatchEvent(new ChangeEvent(new Value(length), 'count'));
				dispatchEvent(new ChangeEvent(getValue('properties'), 'properties'));
			}
			catch(e:*)
			{
		...
		private var __action:ClipsValueAction;
