...
*/
	public class FeedSource extends RemoteSource
	{
		...
		override protected function _loadItemsFromData(data:String):void
		{
			...
			{
				RunClass.MovieMasher['msg']('_loadItemsFromData ' + e)
			}
			// feed does not page
			_more = false;		
		}
		...
		private function __itemDuration(item:IItem):Number
		{
			...
			try
			{
				...
				//if (! seconds) RunClass.MovieMasher['msg']('Could not parse duration ' + duration);
			}
			catch(e:*)
			{
				RunClass.MovieMasher['msg']('__itemDuration ' + e);
			}
			return seconds;		
		}
		private function __tagFromItem(item:IItem):XML
		{
			...
			{
				RunClass.MovieMasher['msg']('__tagFromItem ' + e + ' ' + item );
			}	
			return tag;
		}
