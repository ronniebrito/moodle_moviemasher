...
class MovieMasher
{
	...
	static function &arrayFromXML($xml, $tag_name = '')
	{
		$a = array();
		if ($tag_name) 
		{
	...
	function getOption($k = '')
	{
		...
		if ($k && isset($this->_options[$k]))
		{
			...
			{
				// support retrieval of host from file or URL
				if (strpos($val, '/') !== FALSE) 
				{
					$val = file_get($val);
	...
	function getVersion()
	{
		...
		{
			$this->__version = file_get(dirname(__FILE__) . '/VERSION.txt');
	...
	function log($s, $priority = '')
	{
		...
		if (! empty($this->_options['DirLog']))
		{
			...
			if (file_exists($this->_options['DirLog']))
			{
				...
				$path = $this->_options['DirLog'] . 'log_' . date('Y-m-d') . '.txt';
	...
	static function &parse($template_path, $template_data = array())
	{
