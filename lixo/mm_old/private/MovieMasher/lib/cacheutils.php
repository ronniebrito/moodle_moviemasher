...
*/
include_once(dirname(__FILE__) . '/floatlibs.php');
include_once(dirname(__FILE__) . '/shellutils.php');
...
function flush_cache_kb($dir, $kbs_to_flush)
{
	...
	if ($result)
	{
		...
		foreach($lines as $line)
		{
			...
			if (! $cached) // try to determine from modification time
			{
				$cached = date('Y-m-d H:i:s', filemtime($dir));
			}
			if ($cached)
			{
...
function frame_file_path($file_path, $dimensions, $fps)
{
	...
	return $file_path . $dimensions . 'x' . $fps . '/';
}
