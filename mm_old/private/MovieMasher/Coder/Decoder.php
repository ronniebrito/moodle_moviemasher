...
class MovieMasher_Coder_Decoder extends MovieMasher_Coder
{
	...
	{	
		...
		for ($i = 0; $i < $clip_count; $i++)
		{
			...
			if ($clip['track'] < 0) 
			{
				...
				if (! floatgtre($clip['start'], floatval(0))) continue;
			}
				...
				case 'image':
				{
					...
					if ($parent != NULL) 
					{
						...
						if (sizeof($children))
						{
							...
							if ( ! ( ($first_child == $child) || ($first_child->asXML() == $child->asXML()))) return $neg_one;
						}
					}
				}
			}
	...
	function __buildAudio()
	{
		...
		foreach($this->__mashData['cache_urls'] as $url => $true)
		{
			...
			$result = cache_url($url, $this->_options['DirCache']);
			if (! $result) throw new RuntimeException('Could not cache media file: ' . $url);
