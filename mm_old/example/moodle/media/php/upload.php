...
{	
	...
	//$media->moviemasher_id = $moviemasher->id;
	$media->mash_id = $mash->id;	
	...
	$media_id = insert_record('moviemasher_media', $media);
	$dest_filepath = $CFG->dataroot.'/'.$moviemasher->course.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$media_id.'.'.$extension;	
	move_uploaded_file($file['tmp_name'],$dest_filepath);
	$media = get_record('moviemasher_media', 'id', $media_id);	
	...
	// generate thumbnail	
	exec('ffmpeg -i '. $CFG->dataroot.'/'.$moviemasher->course.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$media_id.'.'.$extension.' -an -ss 00:00:01 -an -r 1 -vframes 1 -y '. $CFG->dataroot.'/'.$moviemasher->course.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$media_id.'.jpg');	
	//$err = $duration;
	$media->name = $file['name'];
	$media->extension = $extension;
	$media->duration = $duration;	
	update_record('moviemasher_media', $media);	
