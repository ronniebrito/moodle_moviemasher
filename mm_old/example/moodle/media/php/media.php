...
print '<moviemasher>' . "\n";	
	...
	$videos = get_records_select('moviemasher_media', "mash_id=". $mash->id . " and type = 'video'",  " id DESC ");
	//print_r($videos);
	...
	// busca os registros de iamgens
	$images = get_records_select('moviemasher_media', "mash_id=". $mash->id . " and type = 'image'",  " id DESC ");
	foreach($images as $image){		
		print '<media group="image" type="image" id="IMG'.$image->id.'" label="'.$image->name.'"';
		print ' source="'. $CFG->wwwroot.'/file.php/'.$course->id.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$image->id.'.'.$image->extension.'"'."\n";
		print ' url="'. $CFG->wwwroot.'/file.php/'.$course->id.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$image->id.'.'.$image->extension.'"'."\n";
		print ' icon="'. $CFG->wwwroot.'/file.php/'.$course->id.'/moddata/moviemasher/'.$cm->id.'/'.$mash->user_id.'/'.$image->id.'.'.$image->extension.'"'."\n";
		print ' fill="scale" '."\n";
		print '/>'."\n";	
	}	
