...
*/
	public class Mash extends Propertied implements IMash
	{
		...
		public function buffer(first:Number, last:Number, mute:Boolean, rebuffer:Boolean):void
		{
			...
			for (var ob:* in clips) // does not included embedded effects!!
			{
				clip = ob;
		...
		public function buffered(first:Number, last:Number, mute:Boolean, rebuffer:Boolean):Boolean
		{
			...
			try
			{
				...
				else RunClass.MovieMasher['msg'](this + '.buffered ' + first + '->' + last);
				*/
			}
			catch(e:*)
			{
		...
		public function clipsInRange(first:Number, last:Number):Array
		{
			...
			for (var i:uint = 0; i < z; i++)
			{
				...
				for (j = 0; j < y; j++)
				{
					...
					{
						clips.push(clip);
						/* DON'T add embedded effects for non audio clips
						if (i) 
						{
		...
		public function clipsInOuterTracks(first:Number, last:Number, ignore:Array = null, track:int = 0, count:int = 0, type:String = ''):Array
		{
			...
			//RunClass.MovieMasher['msg'](this + '.clipsInOuterTracks ' + track_clips);
			...
			for (i = 0; i < z; i++)
			{
				...
				clip_start = clip.getValue(ClipProperty.START).number;// clip.startFrame;
				if (clip_start >= end_time)
				{
					break;
				}
				clip_length = clip.getValue(ClipProperty.LENGTH).number;
		...
		*/
		...
		public function set volumeLevel(iNumber:Number):void
		{
			__volume = iNumber;
		}
		...
		private function __applyClips(v_clips:Array):Boolean
		{
			...
			try
			{
				...
				if (to_clip != null)
				{
					...
					to_clip.setFrame(__displayFrame);
				}
				else
				{
					sprite = new Sprite();
					RunClass.DrawUtility['fillBox'](sprite.graphics, - __bitmapSize.width / 2,  - __bitmapSize.height / 2,  __bitmapSize.width,  __bitmapSize.height, RunClass.DrawUtility['colorFromHex'](backColor));
				...
				if (from_clip != null)
				{
					...
					from_clip.setFrame(__displayFrame);
				}
				else
				{
					sprite = new Sprite();
				...
				__canvasSprite.applyTransition(transition_clip, __displayFrame);
				did_draw = true;
			}
			catch(e:*)
			{
		...
		private function __clipAudioFrame():Number
		{
			...
			{
				RunClass.MovieMasher['msg'](this + '.__clipAudioFrame', e);
			}	
		...
		private function __displayBufferedFrame(now:Number):Boolean
		{
			...
			try
			{
				...
				if (__paused || (__frame != now))
				{
					...
					if ((__bitmapSize != null) && (__canvasSprite != null))
					{
						__canvasSprite.removeContent();
						//if (__clipsChanged) __clearContainers();
						...
						if (__lengthFrame)
						{
							...
							did_draw = ((v_clips.length == 3) ? __applyClips(v_clips) : (v_clips.length ? __applyClip(v_clips[0]) : true));

							if (did_draw) __canvasSprite.applyEffects(__playingEffectClips(), __displayFrame);
							else if (__moving)
							{
					...
					{
						RunClass.MovieMasher['msg'](this + '.__displayBufferedFrame LOCATION', e);
					}	
				...
				//RunClass.MovieMasher['msg'](this + '.__displayBufferedFrame ' + now);
			}
			catch(e:*)
			{
				RunClass.MovieMasher['msg'](this + '.__displayBufferedFrame ' + tag.toXMLString(), e);
			}			
		...
		private function __playingAudioClips():Array
		{
			...
			{
				RunClass.MovieMasher['msg'](this + '.__playingAudioClips', e);
			}
			return array;
		}
		...
		private function __playingEffectClips():Vector.<IClipEffect>
		{
			...
			//if (array.length) RunClass.MovieMasher['msg'](this + '.__playingEffectClips '  + array);
			return array;
		}
		...
		private function __positionCanvas():void
		{
			if (__metrics != null)
			{
				...
				{
					var first_time:Boolean = ! (__canvasSprite.displayObject.x || __canvasSprite.displayObject.y);
					__canvasSprite.displayObject.x =  (__metrics.width / 2);
					__canvasSprite.displayObject.y =  (__metrics.height / 2);
					if ((! first_time) && (__goingFrame == -1)) gotoFrame(__frame);
				}
		...
		private function __setClipVolume(clip:IClip):void
		{
			try
			{
				...
				clip.volumeLevel = clip_volume;
			}
			catch(e:*)
			{
				RunClass.MovieMasher['msg'](this + '.__setClipVolume', e);
			}
		}
		...
		private function __xml():XML
		{
			...
			try
			{
				...
				delete result.clip;
				delete result.media;
				if (result.option.length()) delete result.option;
			//RunClass.MovieMasher['msg'](this + '.__xml delete clip ' + result.toXMLString());
								
			//	if (result.clip.length()) RunClass.MovieMasher['msg'](this + '.__xml was not able to delete ' + result.clip.length() + ' clip tags');
				...
				result.@quantize = getValue(MashProperty.QUANTIZE).number;
				...
				if (z)
				{
					for (var i:uint = 0; i < z; i++)
					{
						type = __trackTypes[i];
						track_clips = __tracks[type];
						//if (type != ClipType.VIDEO) track_clips.sort(sortByTimeTrack);
						y = track_clips.length;
						if (y)
						{
							for (j = 0; j < y; j++)
							{
								...
								//RunClass.MovieMasher['msg'](this + '.__xml ' + clip_tag.toXMLString());
								...
								//if (! result.child('clip').length()) RunClass.MovieMasher['msg'](this + '.__xml with no clip tags but ' + y + ' ' + type + ' clips ' + (clip_tag == null ? 'null tag' :clip_tag.childIndex() + ' ' + clip_tag.toXMLString()));
							}
						}
					}
				}
