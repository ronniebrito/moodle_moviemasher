...
*/
		...
		public static function colorize(display:DisplayObject, color:Number):void
		{
			var newColorTransform:ColorTransform;
			newColorTransform = display.transform.colorTransform;
			newColorTransform.color = color;
			display.transform.colorTransform = newColorTransform;
		}
...
*/
		...
		public static function setLine(graphics:Graphics, thickness:Number = 0, color:Number = 0, alpha:Number = 100)
		{
...
*/
		...
		public static function shadowBox(options:IValued, sprite:Sprite, prefix:String = ''):void
		{
			...
			try
			{
				...
				var angle:Number = options.getValue(prefix + 'angle').number;
				var border:Number = options.getValue(prefix + 'border').number;
				var bordercolor:String = options.getValue(prefix + 'bordercolor').string;
				var color:String = options.getValue(prefix + 'color').string;
				var curve:Number = options.getValue('curve').number;
				var grad:Number = options.getValue(prefix + 'grad').number;
				...
				sprite.filters = ((shadow || shadowcolor.length) ? [new DropShadowFilter(shadow, 45, RunClass.DrawUtility['colorFromHex'](shadowcolor), 1, shadowblur, shadowblur, shadowstrength, 3)]:[]);
			}
			catch(e:*)
			{
