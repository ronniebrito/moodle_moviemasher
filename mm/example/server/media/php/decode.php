...
The script saves out a copy of the mash (file must permit writing by web process):
	PathMedia/{authenticated_userid()}/$id.xml - read by Movie Masher Applet and Server
...
if (! $err)
{
	...
	if ($uploads_locally && (! $host)) $err = 'Configuration option Host or HostLocal required';
}
...
if (! $err)
{
	...
	{
		if ($renders_locally) $options['CoderFileURL'] .= substr($dir_host, 1) . $path_media . $mash_media_id . '/' . $coder_filename . '.' . $decoder_extension;		
		else 
		{
