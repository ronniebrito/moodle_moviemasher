...
made within the system. The Transcoder API enforces this principle for all requests it
...
enable complex interactions with S3, SQS and similar web services. Supported Input types
...
Outputs can contain runtime references like 'Job.Status', 'Job.PercentDone' or even
...
<MovieMasher>
	<Job>
		...
		<Output>
			...
			<Transfer>
				<Host>example.com</Host>
				<Path><![CDATA[status.cgi?p={Job.PercentDone}&d={Transfer.Date}]]></Path>
				<Method>get</Method>
			</Transfer>
		</Output>
		<Input>
...
Job.Output.2.Transfer.Path:status.cgi?p={Job.PercentDone}&d={Transfer.Date}
