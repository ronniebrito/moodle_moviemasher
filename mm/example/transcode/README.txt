...
transcode:
	...
	media:
		php:
			addmedia.php: adds tag for transcoding to user's media.xml file
			choose.php: handles initial screening of uploads
			decode.php: initiates mash decoding (rendering) job
			decoded.php: receives decoded file from transcoder when S3 not used
			decoding.php: monitors a decoding job
			encode.php: initiates media encoding (preprocessing) job
			encoded.php: receives encoded file from transcoder when S3 not used
			encoding.php: monitors an encoding job
			error.php: receives notice when transcoder encounters an error 
			include:
				...
				mimeutils.php - provides MIME determination (via PEAR's MIME_Type)
			...
			progress.php: receives progress data for jobs posted to SQS interface
			save.php: stores mash XML to user's directory and adds to their media.xml
			upload.php: handles upload of media file when S3 not used
		...
		xml:
			...
			panel_record.xml: contains panel tags manifesting recorder controls
	...
	private: for security, should be moved to a directory OUTSIDE web server's root
		moviemasher.ini: configuration for PHP scripts
...
* Change the LOCATION OPTIONS in the private/moviemasher.ini file, and probably others
...
* OR if using your Amazon Security Credentials to sign requests instead:
	...
	Launch Transcoder with Access Key ID and Secret Access Key (see README-Transcoder.txt)
...
* Check out README-Transcoder.txt for information about launching Transcoder AMI
