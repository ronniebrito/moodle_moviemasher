...
if (! $err)
{
	$attributes = '';
	$id = id_unique();
	if ($file_is_url) $file_url = $file_name;
	else $file_url = 'http://' . ($file == 'Local' ? $host : $host_media) . '/' . $path_media . $id . '/' . $encoder_original_filename . '.' .  $extension;
	if ($file_is_url || ($file != 'Local'))
	{
		if ($file == 'S3')
		{
			...
			if ($log_transcoder_requests) log_file('S3 Upload: ' . $attributes, $dir_log);
		}
		$attributes .= ' url="media/php/encode.php?label=' . $file_label . '&amp;';
	}
	else
	{
		$attributes .= ' upload="media/php/upload.php?u=' . auth_userid() . '&amp;';
	}
