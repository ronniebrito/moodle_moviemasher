...
include_once(dirname(dirname(__FILE__)) . '/Daemon.php');
include_once(dirname(dirname(__FILE__)) . '/lib/urlutils.php');
include_once(dirname(dirname(__FILE__)) . '/lib/cryptutils.php');
include_once(dirname(dirname(__FILE__)) . '/lib/idutils.php');
include_once(dirname(dirname(__FILE__)) . '/lib/dateutils.php');
...
{    
	...
	function processRequest($request)
	{
		...
		switch ($method)
		{
			...
			case 'post':
				break;
			default: throw new InvalidArgumentException('Only get or post supported for method');
			...
			case 'decode':
			{
				if (sizeof($split_path) == 2)
				{
					if ($this->_options['DirCache'])
					{
						...
						{
							$response = file_get($progress_path);
						}					
