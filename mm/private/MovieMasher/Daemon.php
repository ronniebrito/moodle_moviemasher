...
include_once(dirname(__FILE__) . '/lib/fileutils.php');
...
class MovieMasher_Daemon extends MovieMasher
{
	...
	static function jobType($job_xml)
	{
	...
	function _populateDefaults()
	{
		...
		$this->_configDefaults['LogRequests'] = array(
			'value' => 'BOOLEAN',
			'description' => "Whether or not to add requests we receive to the log.",
			'default' => 1,
		);
		$this->_configDefaults['LogResponses'] = array(
			'value' => 'BOOLEAN',
			'description' => "Whether or not to add responses we generate to the log.",
			'default' => 1,
		);
	...
	function _processJob()
	{
		...
		if (! set_file_info($progress_path, 'cached', gmdate("Y-m-d H:i:s"))) throw new RuntimeException('Could not set cached info: ' . $progress_path);
	}
    ...
    * @return bool
    */
      $oldPid = file_get($this->_options['DirPID'] . 'MovieMasher_' . $this->_options['module'] . '_' . $this->_options[$this->_options['module']] . '.pid');
		...
		if (! ($this->_pauseWhileProcessing && $this->__jobProcessing()))
		{
			if (empty($this->_jobID))
			{
				...
				if ($this->_jobXML)
				{
					...
					if ($this->_options['LogRequests']) $this->log('RECEIVING Request: ' . $this->_jobXML->asXML());
