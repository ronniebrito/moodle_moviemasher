...
*/
	public class Player extends Control implements IDrop
	{
		...
		public function set mash(iMash:IMash):void
		{
			try
			{
				if (__mash != iMash)
				{
					...
					if (__mash != null)
					{
						//RunClass.MovieMasher['msg'](this + '.mash'); 
						__saveXML();
		...
		private function __mashChange(event:ChangeEvent):void
		{
			...
			switch (event.property)
			{
				...
				case PlayerProperty.DIRTY:
					...
					{
						//RunClass.MovieMasher['msg'](this + '.__mashChange ' + event.property + ' ' + event.value.string); 
						__saveXML();
		...
		{
			//RunClass.MovieMasher['msg'](this + '.__saveXML');
		...
		private var __mashVolume:Number;
