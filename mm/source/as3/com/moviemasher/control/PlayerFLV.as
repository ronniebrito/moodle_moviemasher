...
*/
	public class PlayerFLV extends Control implements IPlayer
	{
		...
		override public function getValue(property:String):Value
		{
			...
			switch(property)
			{
				...
				case PlayerProperty.COMPLETED:
					...
					//RunClass.MovieMasher['msg'](this + '.getValue ' + property + ' ' + value.string + ' ' + __seekingTime);
					break;
		...
		private function __dispatchLocationEvent():void
		{
			...
			dispatchEvent(new ChangeEvent(getValue(PlayerProperty.COMPLETED), PlayerProperty.COMPLETED));
		...
		private function __seekTimed(event:TimerEvent):void
		{
			try
			{
				if (__videoPlayer.stateResponsive)
				{
					...
					else __videoPlayer.seek(__seekedTime);
				}
			}
			catch(e:*)
			{
		...
		private function __setAudio(boolean:Boolean):void
		{
			if (__audio != boolean)
			{
				...
				dispatchEvent(new ChangeEvent(getValue('audio'), 'audio'));
			}
		}
		private function __setStalled(boolean:Boolean):void
		{
			if (__stalled != boolean)
			{
				...
				__stalled = boolean;
				dispatchEvent(new ChangeEvent(getValue(MashProperty.STALLING), MashProperty.STALLING));
			}
		}
		private function __setLabel(s:String):void
		{
			...
			{
				__label = s;
				dispatchEvent(new ChangeEvent(getValue(MediaProperty.LABEL), MediaProperty.LABEL));
			}
		}
		...
		private function __setURL(url_string:String):void
		{
			try
			{
				...
				dispatchEvent(new ChangeEvent(getValue('url'), 'url'));
		...
		private function __makeRequest():void
		{
			try
			{
				...
				{
					__setStalled(true);
					if (__audio) __sound.load(new URLRequest(url_string));
		...
		private function __playerComplete(event:VideoEvent):void 
		{
			//RunClass.MovieMasher['msg'](this + '.__playerComplete ' + event.state);
		...
		*/
		...
		//	RunClass.MovieMasher['msg'](this + '.__videoPlayerData ' + __objectString(event.info));
		...
		{
			//RunClass.MovieMasher['msg'](this + '.__audioPlayerData ' + event);
		...
		private function __videoPlayerStateChange(event:VideoEvent):void 
		{
			...
			switch (event.state)
			{
				...
				case VideoState.PAUSED:
					...
					else dispatchEvent(new Event('stopped'));
