...
*/
	public class Trimmer extends Slider
	{
		...
		override protected function _mouseDrag():void
		{
			try
			{
				var is_end:Boolean = (__pressClip == _displayedObjects.trimicon);
		...
		override protected function _release():void
		{
			try
			{
				var is_end = (__pressClip == _displayedObjects.trimicon);
				var percent:Number = __trimPercent(is_end);
				__values[is_end ? 1 : 0] = percent;
				super._release();
			}
		...
		private function __roll(xPos:Number, yPos:Number):void
		{
			...
			if (display_object != _displayedObjects.icon)
			{
				prefix = 'trim';
			}
			else
		...
		override protected function _update() : void
		{
			...
			if (root != null)
			{
				var mc:DisplayObject = mc = _displayedObjects['disicon'];
				if (mc != null) 
				{
				...
				__roll(root.mouseX, root.mouseY);
			}
