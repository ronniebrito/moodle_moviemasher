	...
	public class RemoteSource extends Source
	{
		...
		protected function _parsedURL():String
		{
			...
			try
			{
				...
				if (url.indexOf('{') != -1)
				{
					if ((RunClass.ParseUtility != null) && (RunClass.ParseUtility['optionBrackets'] != null))
					{
						url = RunClass.ParseUtility['optionBrackets'](url);
					}
				}
				if (url.indexOf('{') != -1)
				{
					url = RunClass.ParseUtility['brackets'](url, this);
				}
				if (url.indexOf('{') != -1)
				{
				...
				//RunClass.MovieMasher['msg'](this + '._parsedURL ' + url);
			}
			catch(e:*)
			{
