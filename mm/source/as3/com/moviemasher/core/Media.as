		...
		override protected function _parseTag():void
		{
			...
			switch(__type)
			{
				case ClipType.VIDEO:
					...
					_defaults.begin = '1';
					_defaults.zeropadding = String(String(Math.floor(getValue(MediaProperty.FPS).number * (getValue(MediaProperty.BEGIN).number + (getValue(MediaProperty.INCREMENT).number * getValue(MediaProperty.DURATION).number)))).length);
